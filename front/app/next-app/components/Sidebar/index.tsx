    ...
    return childNodes
      ...
      .map((child: any, index: any) => {
        ...
        const hasGrandchildren =
          child.content.content && child.content.content.length >= 2;
        ...
        return hasGrandchildren ? (
          ...
          >
            {child.content.content.map(
              (grandchild: any, gIndex: any) =>
                grandchild.content.content[0] && (
                  <div key={gIndex}>{grandchild.content.content[0].text}</div>
                )
            )}
          </div>
        ) : (
          ...
          >
            <p className="font-semibold text-gray-700">
              {typeToJapanese[child.type.name]}
            </p>
    ...
    <div className="bg-white shadow-lg rounded-lg border border-gray-200 p-4 mx-2 my-4">
      ...
      <div>
        ...
        {node.content && node.content.size > 0 && (
          <div>{renderChildNodes(node.content.content, node.pos + 1)}</div>
        )}
      </div>
    </div>
  );
